...
int main(int argc, char **argv) {
    ...
    int mtu = (argc >= 4) ? atoi(argv[3]) : 1500;

    int fd = tun_create(ifname, sizeof(ifname));
    if (fd < 0) return 1;

    LOG_INFO("Created TUN: %s (fd=%d)", ifname, fd);

    char ip[32], mask[32];
    cidr_to_addr_mask(cidr, ip, sizeof(ip), mask, sizeof(mask));

    if (if_set_addr_netmask(ifname, ip, mask) < 0) {
        LOG_ERROR("Failed to set IP/netmask on %s", ifname);
        close(fd); return 1;
    }

    if (if_set_mtu(ifname, mtu) < 0) {
        LOG_ERROR("Failed to set MTU on %s", ifname);
        close(fd); return 1;
    }

    if (if_set_flags(ifname, IFF_UP | IFF_RUNNING, 1) < 0) {
        LOG_ERROR("Failed to bring %s up", ifname);
        close(fd); return 1;
    }

    LOG_INFO("%s is UP with %s/%s, MTU %d.", ifname, ip, mask, mtu);
    LOG_INFO("Reading packets from TUN. Press Ctrl+C to exit.");

    // Simple read loop: print packet sizes. You can write() here to inject packets to kernel.
